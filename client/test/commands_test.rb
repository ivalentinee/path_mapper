...
class CommandsTest < Minitest::Test
  ...
  def test_reports_every_asset_it_uploads
    out = StringIO.new
    PathMapper::Commands.new(adventure_blob, @server, PathMapper::Progress.new(out)).adventure
    lines = out.string.lines.grep(/\.\.\. ok$/)

    assert_equal @server.assets.size, lines.size
    assert(lines.all? { |line| line.match?(/\(\d+(\.\d+)? [KMG]?B\)/) }, out.string)
  end

  def adventure_blob
    PathMapper::Blob.open(fixture('adventures/tt0001-0000000001-adventure-1.zip'))
  end

  # A plug-in dialog shows the exception and nothing else, so the asset has to be
  # named in the message as well as on the progress line.
  def test_names_the_asset_an_upload_failed_on
    refusing = Object.new
    def refusing.store_asset(_name, _bytes) = raise(PathMapper::Server::Failed, 'connection reset')

    error = assert_raises(PathMapper::Server::Failed) do
      PathMapper::Commands.new(adventure_blob, refusing).adventure
    end

    assert_match(/\AUploading \S+: connection reset\z/, error.message)
  end
  ...
  # Both fixture scenes use the same map. Declaring it twice is harmless but
  # pointless, and it makes any count of what was sent wrong.
  def test_declares_each_entity_once
    ids = adventure.map { |command| command['id'] }
