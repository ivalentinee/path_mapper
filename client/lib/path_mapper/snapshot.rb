...
module PathMapper
  ...
  class Snapshot
    ...
    def self.write(server, directory, progress = Silence.new)
      entities = progress.step('reading entities') { server.entities.fetch('entities') }
      state = progress.step('reading game state') { server.state }
      assets = server.fetch_assets(asset_paths(entities), progress)
